...
var infobox2 = document.querySelector('.infobox2'); // 登录按钮绑定点击事件
...
btn.onclick = function () {
  ...
  xhr.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded'); // 获取数据

  var data = "username=".concat(username.value, "&password=").concat(password.value); // 发送获取到的数据data

  xhr.send(data); // 状态处理函数

  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4) {
      if (xhr.status >= 200 && xhr.status < 300) {
        // 获取响应的数据
        var res = xhr.responseText; // JSON字符串转成对象

        res = JSON.parse(res);
        console.log(res); // 解构赋值

        var _res = res,
            errCode = _res.errCode,
            message = _res.message,
            token = _res.token; // 取消弹出层

        layer.close(loadid); // errCode===0 代表登录成功

        if (errCode === 0) {
          // 设置token,存储在localStorage中
          localStorage.setItem('token', token); // 设置cookie username

          setCookie('username', username.value, 7200); // layer插件 登录的成功弹出层

          layer.msg(message, {
            icon: 1,
            time: 2000 // 2秒之后执行回调函数，跳转到首页
