...
$(function () {
  $('#addToCartBtn').on('click', function () {
    ...
    var num = sp_num.value; // 构建要添加到购物车的商品对象
    ...
    $.ajax({
      ...
      success: function success(response) {
        ...
        var cartItems = localStorage.getItem('cartItems') ? JSON.parse(localStorage.getItem('cartItems')) : []; // 将新的购物车项添加到数组中
        // unshift追加到数组第一位

        cartItems.unshift(goodsData); // 存储更新后的购物车数据到 localStorage

        localStorage.setItem('cartItems', JSON.stringify(cartItems)); // 判断用户是否已经登录

        if (localStorage.getItem('token')) {
          var message2 = '加入购物车成功'; // 取消加载层
          ...
          layer.msg(message2, {
            ...
            if (token) {
              // 获取购物车数据
              var _cartItems = localStorage.getItem('cartItems') ? JSON.parse(localStorage.getItem('cartItems')) : []; // 顶部显示购物车内商品件数
