...
var leftimg = document.querySelector('.leftbox img')
...
function magnifier() {
  ...
  leftbox.onmousemove = function (e) {
    ...
    var top_distance = e.pageY - leftbox.offsetTop - parseInt(mengbanceng.offsetHeight / 2)

    //设置当前蒙版层最大的移动距离
    var maxX = leftbox.clientWidth - mengbanceng.clientWidth
    var maxY = leftbox.clientHeight - mengbanceng.clientHeight
    ...
    rightimg.style.top = -(rightimg.offsetHeight - rightbox.offsetHeight) * bly + 'px'

    // // 右边图片的放大效果
    var scale = 1.5; // 设置放大倍数
    rightimg.style.transform = `scale(${scale})`;
    rightimg.style.transformOrigin = `${blx * 100}% ${bly * 100}%`;
...
$(function () {
  ...
  // window.location.search，你可以获取当前页面 URL 中的查询参数部分，包括问号 ? 后面的内容。
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $('.now_price1').text(data.data[0].price)
      // //  jQuery 库的attr方法,可以设置指定元素的属性值，这里设置img标签的src属性，即图片路径
      ...
      $('.show4').attr('src', data.data[0].url3).attr('data-imgurl', data.data[0].url3)

       // 切换图片的函数
      function change(m) {
        var imgurl = $('.show' + m).attr('data-imgurl')
        leftimg.setAttribute('src', imgurl)
        rightimg.setAttribute('src', imgurl)
        // 切换图片后再次调用放大镜函数
        magnifier()
      }
